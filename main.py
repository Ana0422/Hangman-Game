...
def input_char_in_word(selected_word, input_char, current_word_state, attempts_remaining):
    if input_char in selected_word:
        current_word_state = change_current_word_state(
            selected_word, input_char, current_word_state)
    else:
...
def print_current_state(current_word_state, attempts_remaining):
    # It will print current status of the game i.e alphabets guessed so far and attempts left
...
def play_game(attempts=5):
    ...
    # It will store the value of randomly selected word
    selected_word = pick_random_word()

    # It will show present state of the word
    current_word_state = ""
    for i in selected_word:
        if i == " " or i == "a" or i == "e" or i == "i" or i == "u" or i == "o":
    ...
    while True:
        ...
        game_end_checking = check_game_status(
            selected_word, current_word_state, attempts_remaining)

        if game_end_checking == False:
            break
